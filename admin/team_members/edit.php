...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE team_members SET full_name = ?, position = ?, bio = ?, photo = ?, portfolio = ?, is_active = ?, order_index = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?");
    $stmt->bind_param("sssssiii", $full_name, $position, $bio, $photo_path, $portfolio, $is_active, $order_index, $team_member_id);
    ...
    if ($stmt->execute()) {
        ...
        $_SESSION['team_member_error'] = "Error updating team member: " . $conn->error;
    }
}
?>
                ...
                <form method="POST" action="" enctype="multipart/form-data">
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" id="position" name="position" value="<?php echo htmlspecialchars($team_member['position']); ?>" required>
                        </div>
                    </div>
                    <div class="mb-3">
                        <label for="bio" class="form-label">Bio</label>
                        <textarea class="form-control" id="bio" name="bio" rows="5"><?php echo htmlspecialchars($team_member['bio']); ?></textarea>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
